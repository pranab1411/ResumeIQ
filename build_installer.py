...
def main():
    ...
    if iscc_found:
        ...
        run_step("Compiling Inno Setup Executable (ResumeIQ_Setup_v2.1.exe)", iscc_cmd, cwd=base_dir)
        setup_exe = os.path.join(base_dir, "Output", "ResumeIQ_Setup_v2.1.exe")
        if os.path.exists(setup_exe):
            print(f"\n[SUCCESS] Installer created at: {setup_exe}")
    else:
        print("\n" + "="*60)
        print(" [INFO] Inno Setup Compiler (ISCC.exe) not found on system PATH.")
        print("        To build the single ResumeIQ_Setup_v2.1.exe file:")
